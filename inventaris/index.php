...
if (isset($_GET['hapus'])) {
    $id = (int)$_GET['hapus'];
    $res = $conn->query("SELECT foto FROM barang WHERE id_barang = $id");
    $row = $res->fetch_assoc();
...
$where  = $search ? "WHERE kode_barang LIKE '%$search%' OR nama_barang LIKE '%$search%'" : '';
$result = $conn->query("SELECT * FROM barang $where ORDER BY id_barang DESC");
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <style>
        ...
        .user-badge { background: rgba(255,255,255,.2); border-radius: 20px; padding: 4px 12px; font-size: .85rem; }
    </style>
</head>
<body>
...
<div class="container">
    <?php if (isset($_GET['msg'])): ?>
        <?php $msgs = ['tambah'=>['success','Barang berhasil ditambahkan!'],
            ...
            <div class="col-md-6">
                <div class="input-group">
                    ...
                    <input type="text" name="search" class="form-control"
                           placeholder="Cari kode atau nama barang..." value="<?= htmlspecialchars($search) ?>">
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if ($row['foto'] && file_exists('uploads/'.$row['foto'])): ?>
                                    <?php
                                    $thumb_path = 'uploads/thumb_' . $row['foto'] . '.jpg';
                                    $img_src = file_exists($thumb_path) ? $thumb_path : 'uploads/' . $row['foto'];
                                    ?>
                            ...
                            <td class="text-center">
                                ...
                                <a href="index.php?hapus=<?= $row['id_barang'] ?>"
